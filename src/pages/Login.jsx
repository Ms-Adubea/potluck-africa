...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.isApproved === false) {
        setErrors({ 
          general: 'Your account is awaiting approval by an admin. You will be notified once approved.' 
        });
        return;
      }
      ...
      if (errorMessage?.includes('approval') || errorMessage?.includes('pending')) {
  ...
  const handleGoogleSignIn = () => {
    try {
      ...
      setErrors({
        general: 'Unable to initiate Google sign-in. Please try again.'
      });
      setIsLoading(false);
    }
  };
        ...
        <div className="px-8 pb-8">
          ...
          <div className="space-y-3">
            <button 
              ...
              disabled={isLoading}
              className="w-full flex items-center justify-center px-4 py-3 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
