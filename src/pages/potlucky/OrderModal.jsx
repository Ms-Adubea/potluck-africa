...
//   // Payment method options - Fixed to match backend
...
//   // Reset form when modal opens/closes and pre-fill user email
//   useEffect(() => {
//     if (isOpen) {
...
//         paymentMethod,
//         notes: notes.trim(),
//         customerEmail: customerEmail.trim() // Add email to order data
//       };
...
//       if (paymentMethod === 'card' || paymentMethod === 'momo') {
//         try {
//           // Create payment for card and mobile money
//           const paymentPayload = {
//             orderId: response.order._id,
//             amount: response.order.totalPrice,
//             email: customerEmail.trim(),
//             paymentMethod: paymentMethod,
//             ...(paymentMethod === 'momo' && { momo: orderData.momo })
//           };
          
//           paymentData = await apiCreatePayment(paymentPayload);
//         } catch (paymentError) {
...
//           ? 'Your order has been placed. Pay when you pickup!'
...
//       onOrderSuccess && onOrderSuccess(response.order, paymentData);
//       onClose();
...
//                   placeholder={getCurrentUser()?.email ? getCurrentUser().email : "your.email@example.com"}
//                   disabled={loading}
...
//             {/* Mobile Money Details - Show only when momo is selected */}
//             {paymentMethod === 'momo' && (
...
const OrderModal = ({ meal, isOpen, onClose, onOrderSuccess }) => {
  ...
  if (isOpen) {
    ...
    // Only reset email if no current user email exists
...
const validateForm = () => {
  ...
  // Validate mobile money details ONLY when momo is selected
  if (paymentMethod === 'momo') {
    if (!momoDetails.phone.trim()) {
      setError('Please enter your mobile money number');
      return false;
...
// Updated handleSubmit function in OrderModal
const handleSubmit = async () => {
  if (!validateForm()) {
    return;
  }
  ...
  try {
    ...
    // Get email consistently - prioritize customer input, fallback to user email
    const emailToUse = customerEmail.trim() || getCurrentUser()?.email || '';
    
    // Final validation for non-cash payments
    if (paymentMethod !== 'cash' && !emailToUse) {
      loadingSwal.close();
      setError('Email address is required for payment processing');
      return;
    }

    // ✅ FIXED: Remove email from order data - only send fields expected by backend
    const orderData = {
      meal: meal.id,
      quantity,
      pickupTime,
    ...
    // Only initialize payment for non-cash methods
    if (paymentMethod !== 'cash') {
      try {
        ...
        const paymentPayload = {
          orderId: response.order._id,
          amount: response.order.totalPrice,
          email: emailToUse, // ✅ Use email only for payment, not order
          paymentMethod: paymentMethod
        };

        // Add momo details only for mobile money
        if (paymentMethod === 'momo') {
          paymentPayload.momo = {
            phone: momoDetails.phone.trim(),
            provider: momoDetails.provider
...
// Ensure email is passed to Paystack inline popup
if (paymentMethod === 'card' && paymentData.access_code) {
  await initializePaystackPopup({ 
    ...paymentData, 
    email: emailToUse  // <-- Force attach email
  }, response.order);
  return;
}

        
        // Handle Paystack inline popup for card payments
        if (paymentMethod === 'card' && paymentData.access_code) {
          // Load Paystack script and initialize inline payment
          await initializePaystackPopup(paymentData, response.order);
          return; // Exit here as popup will handle the rest
        }
        
        // For other payment methods, continue with redirect if available
...
const initializePaystackPopup = async (paymentData, order) => {
  return new Promise((resolve, reject) => {
    const handler = PaystackPop.setup({
      key: paymentData.public_key || 'your_paystack_public_key', // Replace with your actual public key
